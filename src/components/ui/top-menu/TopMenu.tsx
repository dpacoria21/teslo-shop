    ...
    return (
        <nav className="flex px-5 justify-between items-center w-full">
            ...
            <div className="flex items-center">
                ...
                <Link href={((productsInCart===0 && loaded) ? '/empty': '/cart')} className='mx-2'>
                    <div className='relative'>
                        {
                            loaded && productsInCart !== 0 && ( <span className='absolute text-xs rounded-full px-1 font-bold -top-2 -right-2 bg-blue-700 text-white fade-in'>{productsInCart}</span> ) 
                        }
